...
//		if (YahtzeeMagicStub.checkCategory(dice, categoria)) {
			...
			case FIVES:
			case SIXES:
				score = quantityEquals(dice, categoria);
				break;
...
//		}
	...
	private int sumStraight(int[] dice, int numElements) {
		...
		/*
		 * Converts an array from i.e [ 5,4,2,4,6] to[0,1,0,1,1,1] in other words
		 * show if a number is present in the array if 4 or 5 numbers are
	...
	private int sumEquals(int[] dice, int quantity) {
		...
		/*
		 * Converts an array from i.e [ 5,4,2,4,6] to[0,1,0,2,1,1] for 4 in other
		 * words counts how many times a number is in the array and indicates if
		 * a number is in dice grather o equal than quantity times
		 */
	...
	private int fullHouse(int[] dice) {
		...
		/*
		 * Converts an array from i.e [ 5,4,2,4,5] to[0,1,0,2,2,0] in other words
		 * counts how many times a number is in the array and sum the quantities
		 * grather than one , in the example the sum is 4
		 * example 2 [ 5,4,5,4,5] to [0,0,0,2,3,0] sum equal to 5, and there are full house
		 */
